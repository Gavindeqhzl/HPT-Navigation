...
class DecodeBlock(nn.Module):
    ...
    def forward(self, x, rep_enc):
        """
        x: (B, N, C)  这里的 C 已经是 n_embd （add 模式）或 2*n_embd（concat 模式）
        rep_enc: (B, N, n_embd) 或 None
        """
        # 只在 add_obs_action=True 且提供了 rep_enc 时做“相加”融合
        if self.config.add_obs_action and (rep_enc is not None):
            x = rep_enc + x
...
class AgentLayerDecoder(nn.Module):
    def __init__(self, config, action_dim, n_embd, n_layer, n_head, action_type='Discrete', device='cpu'):
        ...
        # 注意：当 add_obs_action=False 时，入口会 concat 一次，通道变成 2*n_embd，
        # 所以 block 的构造也要对应地用 2*n_embd
        self.blocks = nn.Sequential(*[
            DecodeBlock(config, n_embd if config.add_obs_action else 2 * n_embd, n_head, device)
            for _ in range(n_layer)
        ])

        self.head = nn.Sequential(
            init_(nn.Linear(n_embd if config.add_obs_action else 2 * n_embd, n_embd), activate=True)
        )

    def forward(self, actions, obs_rep):
        # actions: (B, N, act_dim) ; obs_rep: (B, N, n_embd)
        if self.action_type == 'Discrete':
            actions = F.one_hot(actions.squeeze(-1).long(), num_classes=self.action_dim).float()

        x = self.action_encoder(actions)  # (B, N, n_embd)
        x = self.ln(x)

        if not self.config.add_obs_action:
            # 只在这里 concat 一次，得到 (B, N, 2*n_embd)
            x = torch.cat([obs_rep, x], dim=-1)
            rep_for_blocks = None
        else:
            # add 模式不会改通道数
            rep_for_blocks = obs_rep

        # 之后的每层都不再 concat；若是 add 模式则在 block 里相加
        for block in self.blocks:
            x = block(x, rep_for_blocks)
...
class MultiTransRewardDivideModel(nn.Module):
    ...
    def forward(self, states, actions, timesteps, attn_mask=None):
        ...
        if self.config.use_weighted_sum:
            ...
            # 这里是w/o Cooperative (去除协作依赖)的关键步骤
            casual_mask = torch.ones((1, 1, agent_num, agent_num), device=self.device)[:, :, key_len - query_len:key_len, :key_len]
            # casual_mask = torch.eye(agent_num, device=self.device).view(1, 1, agent_num, agent_num)
            # casual_mask = casual_mask[:, :, key_len - query_len:key_len, :key_len]
